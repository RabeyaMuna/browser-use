...
class Registry(Generic[Context]):
	...
	def create_action_model(self, include_actions: list[str] | None = None, page=None) -> type[ActionModel]:
		...
		for name, action in available_actions.items():
			...
			# Build field definitions dict to avoid pyright issues with ** unpacking
			field_defs = {
				name: (
					action.param_model,
			...
			individual_model = create_model(  # type: ignore[call-overload]
